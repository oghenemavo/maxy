<div class="m-login__signup">
	...
	<form class="m-login__form m-form" action="{{ route('create-workflow') }}" method="post">
		<div class="form-group m-form__group">
			...
			<input class="form-control m-input" type="text" placeholder="" value="{{ @$workflow->name }}" required="" name="name">
		</div>
		<div class="form-group m-form__group">
			
            <label>Description:</label>
            <textarea  class="form-control m-input" name="description" rows="5">{{ @$workflow->description }}</textarea>
		</div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        @else
        var url = "{{ route('edit-workflow', @$workflow->id) }}";
        @endif
