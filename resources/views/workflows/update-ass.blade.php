<div class="m-login__signup">
    @php
    if($ass && $isEdit) {
        if($ass->recipient_type == 'USER')
            $recipient = getUserName($ass->recipient_id);
        else if($ass->recipient_type == 'GROUP')
            $recipient = getGroupName($ass->recipient_id);
        else 
            $recipient  =   'Initiator';    
    }
@endphp
	...
	<form class="m-login__form m-form" action="#" method="post">
		<div class="form-group m-form__group">
         ...
         <div class="form-group m-form__group">
            ...
            <textarea  class="form-control m-input" name="template" rows="5" required>{{ @$ass->template }}</textarea>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        @if(!$isEdit)
            var url = "{{ route('update-step-assignee', [ $step_id, null ] ) }}";
        @else
            var url = "{{ route('update-step-assignee', [ $step_id, @$ass->id ] ) }}";
