...
<div class="m-login__signup">
	<form class="m-login__form m-form" action="{{ route('create-user') }}" method="post">
		...
		<div class="form-group m-form__group">
            ...
            {!! Form::select("type", config('constants.user-field-types'), @$field->type, ['class'=>'form-control', 'required', 'id'=>'typeField']) !!}
			
            
		</div>

        <div class="form-group m-form__group collapse" id="vListDiv">
            <div class="m-login__desc">Choose value list:</div>
            {!! Form::select("value_list_id", prepDropdownItemsFromMetadata($vlists,'') , @$field->value_list_id, ['class'=>'form-control']) !!}
            <div class="help-text"><a href="{{ route('value-lists') }}">Create a value list here >>></a></div>
...
$('.select2').select2({dropdownCssClass : 'bigdrop'});
    ...
    function showValueListDiv(){
        ...
        if(type == "Dropdown" || type == "Multiple selection")
            $("#vListDiv").removeClass('collapse');
        else
            $("#vListDiv").addClass('collapse');
    }
